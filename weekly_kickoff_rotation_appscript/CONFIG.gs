...
const CONFIG = {
  ...
  getMany(defaultsByKey) {
    LOGGER.debug("START", { action: "CONFIG.getMany", keys: Object.keys(defaultsByKey || {}) });
    const startMs = Date.now();
    try {
      const defaults = defaultsByKey && typeof defaultsByKey === "object" ? defaultsByKey : {};
      const out = {};
      const keys = Object.keys(defaults);
      if (keys.length === 0) {
        LOGGER.info("SUCCESS", { action: "CONFIG.getMany", keys: 0, durationMs: Date.now() - startMs });
        return out;
      }
      ...
      for (let j = 0; j < keys.length; j++) {
        const key = keys[j];
        out[key] = Object.prototype.hasOwnProperty.call(map, key) ? map[key] : defaults[key];
      }
      LOGGER.info("SUCCESS", { action: "CONFIG.getMany", keys: keys.length, durationMs: Date.now() - startMs });
      return out;
    } catch (e) {
      LOGGER.error("CONFIG.getMany", e.message, e.stack);
      const fallback = {};
      const defaults = defaultsByKey && typeof defaultsByKey === "object" ? defaultsByKey : {};
      const keys = Object.keys(defaults);
      for (let i = 0; i < keys.length; i++) {
        fallback[keys[i]] = defaults[keys[i]];
      }
      return fallback;
    }
  },
...
function ensureBusinessSheets_() {
  ...
  try {
    ...
    if (rotationCareSheet.getLastRow() === 0) {
      rotationCareSheet.getRange(1, 1, 1, 6).setValues([["order", "name", "slackUserId", "email", "active", "luccaUserId"]]);
      rotationCareSheet.getRange(2, 1, 3, 6).setValues([
    ...
    if (rotationSalesSheet.getLastRow() === 0) {
      rotationSalesSheet.getRange(1, 1, 1, 6).setValues([["order", "name", "slackUserId", "email", "active", "luccaUserId"]]);
      rotationSalesSheet.getRange(2, 1, 3, 6).setValues([
    ...
    if (rotationStartSheet.getLastRow() === 0) {
      rotationStartSheet.getRange(1, 1, 1, 6).setValues([["order", "name", "slackUserId", "email", "active", "luccaUserId"]]);
      rotationStartSheet.getRange(2, 1, 2, 6).setValues([
    ...
    ensureRotationLuccaColumn_(rotationCareSheet, "rotationCare");
    ensureRotationLuccaColumn_(rotationSalesSheet, "rotationSales");
    ensureRotationLuccaColumn_(rotationStartSheet, "rotationStart");
    ...
    if (historySheet.getLastRow() === 0) {
      historySheet.getRange(1, 1, 1, 15).setValues([[
        "runAt",
        "mondayDate",
        "team",
        "primaryName",
      ...
      ensureHistoryExtendedColumns_(historySheet);
    }
...
function ensureHistoryExtendedColumns_(sheet) {
  LOGGER.debug("START", { action: "ensureHistoryExtendedColumns_" });
  const startMs = Date.now();
  try {
    if (!sheet) {
      LOGGER.error("ensureHistoryExtendedColumns_", "Missing sheet", "");
      return false;
    }
    const header = sheet.getRange(1, 1, 1, Math.max(15, sheet.getLastColumn())).getValues()[0];
    const updates = [];
    if (String(header[13] || "").trim() !== "runMode") {
      updates.push({ col: 14, value: "runMode" });
    }
    if (String(header[14] || "").trim() !== "runWindowReason") {
      updates.push({ col: 15, value: "runWindowReason" });
    }
    for (let i = 0; i < updates.length; i++) {
      sheet.getRange(1, updates[i].col).setValue(updates[i].value);
    }
    LOGGER.info("SUCCESS", { action: "ensureHistoryExtendedColumns_", updated: updates.length, durationMs: Date.now() - startMs });
    return true;
  } catch (e) {
    LOGGER.error("ensureHistoryExtendedColumns_", e.message, e.stack);
    return false;
  }
}

function ensureRotationLuccaColumn_(sheet, alias) {
  LOGGER.debug("START", { action: "ensureRotationLuccaColumn_", alias: alias });
  const startMs = Date.now();
  try {
    if (!sheet) {
      LOGGER.error("ensureRotationLuccaColumn_", "Missing sheet", "");
      return false;
    }
    const header = sheet.getRange(1, 1, 1, Math.max(6, sheet.getLastColumn())).getValues()[0];
    const hasLuccaHeader = String(header[5] || "").trim() === "luccaUserId";
    if (!hasLuccaHeader) {
      sheet.getRange(1, 6).setValue("luccaUserId");
    }
    LOGGER.info("SUCCESS", {
      action: "ensureRotationLuccaColumn_",
      alias: alias,
      updated: !hasLuccaHeader,
      durationMs: Date.now() - startMs
    });
    return true;
  } catch (e) {
    LOGGER.error("ensureRotationLuccaColumn_", e.message, e.stack);
    return false;
  }
}
