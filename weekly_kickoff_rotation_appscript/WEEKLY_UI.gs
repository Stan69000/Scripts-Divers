...
function getStartWeeklyContext() {
  ...
  const startMs = Date.now();
  try {
    const cached = getCachedJson_("start_weekly_context_v1");
    if (cached) {
      LOGGER.info("SUCCESS", { action: "getStartWeeklyContext", source: "cache", durationMs: Date.now() - startMs });
      return cached;
    }

    const cfg = CONFIG.getMany({
    ...
    const meetUrl = getWeeklyVisioUrlFromCfg_(cfg);
    const remoteBaseUrl = String(cfg.LIEN_REMOTE || "").trim();
    let absenceUrl = "";
    try {
    ...
    const timezone = String(cfg.TIMEZONE || "Europe/Paris");
    ...
    putCachedJson_("start_weekly_context_v1", result, 120);
...
function getWeeklySlideUrlForUi() {
  ...
  try {
    ...
    const mondayIso = Utilities.formatDate(mondayDate, tz, "yyyy-MM-dd");
    const cacheKey = "ui_slide_url_" + mondayIso;
    const cached = getCachedJson_(cacheKey);
    if (typeof cached === "string") {
      LOGGER.info("SUCCESS", { action: "getWeeklySlideUrlForUi", hasUrl: !!cached, source: "cache", durationMs: Date.now() - startMs });
      return cached;
    }

    const slideSlackLink = getWeeklySlideLinkByMondayDate_(mondayDate);
    const slideUrl = extractUrlFromSlackLink_(slideSlackLink);
    putCachedJson_(cacheKey, slideUrl || "", 300);
...
function publishRemoteCode(remoteCode) {
  ...
  try {
    ...
    let message = buildRemoteCodeMessage_(context.mondayDate, safeCode, remoteBaseUrl, visioUrl);
    message = sanitizeRemoteCodeMessage_(message);
    ...
    const ok = code === 200;
    putCachedJson_("start_weekly_context_v1", null, 1);
...
function getWeeklyVisioUrl_() {
  ...
  try {
    ...
    const rawUrl = String(cfg.LIEN_VISIO_WEEKLY || cfg.WEEKLY_MEET_URL || "").trim();
...
function getWeeklyVisioUrlFromCfg_(cfg) {
  LOGGER.debug("START", { action: "getWeeklyVisioUrlFromCfg_" });
  const startMs = Date.now();
  try {
    const source = cfg && typeof cfg === "object" ? cfg : {};
    const rawUrl = String(source.LIEN_VISIO_WEEKLY || source.WEEKLY_MEET_URL || "").trim();
    const url = normalizeMeetUrl_(rawUrl);
    LOGGER.info("SUCCESS", { action: "getWeeklyVisioUrlFromCfg_", hasUrl: !!url, durationMs: Date.now() - startMs });
    return url;
  } catch (e) {
    LOGGER.error("getWeeklyVisioUrlFromCfg_", e.message, e.stack);
    return "";
  }
}

function getCachedJson_(key) {
  LOGGER.debug("START", { action: "getCachedJson_", key: key });
  const startMs = Date.now();
  try {
    const cache = CacheService.getScriptCache();
    const raw = cache.get(String(key || ""));
    if (!raw) {
      LOGGER.info("SUCCESS", { action: "getCachedJson_", hit: false, durationMs: Date.now() - startMs });
      return null;
    }
    const parsed = JSON.parse(raw);
    LOGGER.info("SUCCESS", { action: "getCachedJson_", hit: true, durationMs: Date.now() - startMs });
    return parsed;
  } catch (e) {
    LOGGER.error("getCachedJson_", e.message, e.stack);
    return null;
  }
}

function putCachedJson_(key, value, ttlSeconds) {
  LOGGER.debug("START", { action: "putCachedJson_", key: key, ttlSeconds: ttlSeconds });
  const startMs = Date.now();
  try {
    const cache = CacheService.getScriptCache();
    const cacheKey = String(key || "");
    const ttl = Number(ttlSeconds || 60);
    if (!cacheKey) {
      LOGGER.info("SUCCESS", { action: "putCachedJson_", skipped: "missing_key", durationMs: Date.now() - startMs });
      return false;
    }
    if (value === null || value === undefined) {
      cache.remove(cacheKey);
      LOGGER.info("SUCCESS", { action: "putCachedJson_", removed: true, durationMs: Date.now() - startMs });
      return true;
    }
    cache.put(cacheKey, JSON.stringify(value), ttl);
    LOGGER.info("SUCCESS", { action: "putCachedJson_", stored: true, durationMs: Date.now() - startMs });
    return true;
  } catch (e) {
    LOGGER.error("putCachedJson_", e.message, e.stack);
    return false;
  }
}
...
function buildRemoteCodeMessage_(mondayDateIso, remoteCode, remoteBaseUrl, visioUrl) {
  ...
  try {
    ...
    const message = sanitizeRemoteCodeMessage_(lines.join("\n"));
...
function sanitizeRemoteCodeMessage_(message) {
  LOGGER.debug("START", { action: "sanitizeRemoteCodeMessage_", hasMessage: !!message });
  const startMs = Date.now();
  try {
    ...
    LOGGER.info("SUCCESS", { action: "sanitizeRemoteCodeMessage_", durationMs: Date.now() - startMs });
    return output;
  } catch (e) {
    LOGGER.error("sanitizeRemoteCodeMessage_", e.message, e.stack);
